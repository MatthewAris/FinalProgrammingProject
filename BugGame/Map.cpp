...
m_mapGravity(512.f), m_loadNextMap(false), m_currentState(l_currentState)
{
	...
	while (std::getline(mapFile, line)) {
		...
		if (type == "TILE") {
			...
			if (itr == m_tileSet.end()) {
				std::cout << "! Tile id(" << tileId << ") was not found in the tileset." << std::endl;
				continue;
			}
			sf::Vector2i tileCoords;
			keystream >> tileCoords.x >> tileCoords.y;
			if (tileCoords.x > m_maxMapSize.x || tileCoords.y > tileCoords.y) {
