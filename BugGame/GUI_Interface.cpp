...
GUI_Interface::GUI_Interface(const std::string& l_name, GUI_Manager* l_guiManager)
	...
	m_movable(false), m_beingMoved(false), m_showTitleBar(false), m_focused(false),
	m_scrollHorizontal(0), m_scrollVertical(0), m_contentRedraw(true), m_controlRedraw(true)
{
...
void GUI_Interface::ApplyStyle()
{
	...
	m_titleBar.setFillColor(m_style[m_state].m_elementColour);
	m_visual.m_text.setPosition(m_titleBar.getPosition() + m_style[m_state].m_textPadding);
